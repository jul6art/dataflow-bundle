        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->scalarNode('stimulus_identifier')
                    ...
                    ->cannotBeEmpty()
                ->end()
                ->scalarNode('select2_identifier')
                    ->info('The Stimulus identifier of the APPLICATION\'s select2 controller, attached to the two selects of a filter row in the shipped partial. Like "stimulus_identifier" it is the consumer\'s, derived from where they put the controller file — the three applications of this ecosystem re-export datatable-bundle\'s from assets/controllers/ui/, which yields "ui--select2". Set it to an empty string to keep plain selects: an identifier naming a controller the application never registered is harmless anyway, since Stimulus simply finds nothing.')
                    ->defaultValue('ui--select2')
                ->end()
                ->scalarNode('csrf_token_id')
                    ->info('The CSRF token id validated on the report builder\'s "save" route (D-9bis / lot 2.9). "run" and "export" leak nothing cross-origin and need none; "save" creates or overwrites a report in the actor\'s own account. An application that already validates a token under another id keeps it by setting this instead of renaming two hundred routes.')
                    ->defaultValue('dataflow_report_builder')
                    ->cannotBeEmpty()
                ->end()
                ->arrayNode('limits')
