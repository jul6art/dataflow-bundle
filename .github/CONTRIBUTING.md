# Contributing to `jul6art/dataflow-bundle`

`jul6art/dataflow-bundle` is everything a management application needs to move tabular data
across its own boundary: writing a CSV, an XLSX or a JSON stream, reading one back, and
running a user-composed report over Doctrine entities. It was extracted from two production
applications that between them wrote tabular output in eleven places, in four dialects, and
guarded none of them against spreadsheet formula injection.
...
* **Bug** → open an [issue](https://github.com/jul6art/dataflow-bundle/issues/new/choose)
...
```bash
git clone https://github.com/jul6art/dataflow-bundle.git
cd dataflow-bundle
composer install
```

You need PHP **^8.5** and Composer 2. This bundle also carries a small development
application (`src/Kernel.php`, `.env`, `compose.yaml`), which is how its screens are looked
at without generating a whole project: `docker compose up -d` then `symfony serve`. It is a
harness, not a product — nothing an application installs depends on it.
...
| `test` | PHPUnit (29 test classes today) | by hand |
...
`jul6art/dataflow-bundle` is installed by other projects through Composer, so the version
...
2. **Every value written to a spreadsheet passes the formula guard. No exceptions, no fast
   paths.** This is the defect the bundle was created to end: a cell beginning `=`, `+`,
   `-`, `@`, a tab or a carriage return is a program that runs on the recipient's machine. A
   new writer, a new dialect or a new cell-formatting path ships the test that proves the
   guard is still on it.

3. **A report is a `QueryBuilder`, not a resource.** This bundle deliberately does not
   depend on API Platform: exposing saved reports over HTTP is the application's business.
   Keep that boundary.

4. **A user-composed report only reaches what the application declared.** Entities,
   properties, joins and aggregates all come from an explicit allow-list — never from
   request input reflected into DQL. This is the bundle's sharpest edge: the user composes a
   query, so the composition surface is an attack surface.

5. **Reading a file is reading hostile input.** An import parses something a user uploaded:
   bound the size, refuse external entities and remote references, cap the expansion of a
   compressed sheet, and never let the parsed content choose which entity gets written.

6. **Import and export both respect the caller's access decision**, row by row when the
   application says so. An export is a read of the whole table at once, which makes it the
   easiest place to forget.

7. **Interface strings are translation keys, translated in both `en` and `fr`**
   (`translations/`). A hardcoded string in a template is a bug in this repository.
